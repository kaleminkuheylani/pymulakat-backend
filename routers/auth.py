...
# ═══════════════════════════════════════════════════════════════
# ─── Şifremi Unuttum — 2 adımlı ─────────────────────────────
# ═══════════════════════════════════════════════════════════════


@router.post("/forgot-password/verify-username", response_model=MessageResponse)
async def forgot_password_verify_username(payload: ForgotPasswordStep1Payload):
    """
    Adım 1: Email + username doğrula.
    Başarılıysa 6-haneli kod gönder (password reset için).
    Email enumeration koruması: user yoksa da aynı mesajı döner.
    """
    try:
        sb_admin = get_supabase_admin()
        ...
        ).eq("email", payload.email).maybe_single().execute()

        # Email enumeration koruması: sonucu user'a söylemeden işlemi yap
        if result.data:
            profile = result.data
            # Username karşılaştır (case-insensitive)
            if profile.get("username", "").lower() == payload.username.lower():
                # Doğrulama başarılı → kod gönder
                username = profile.get("username", payload.email.split("@")[0])
                code = _generate_6_digit_code()
                _store_verification_code(sb_admin, payload.email, code)
                ...
                _send_email(
                    to=payload.email,
                    subject=f"🐍 PythonMulakat — Şifre Sıfırlama Kodun: {code}",
                    html=html,
                    text=text,
                )

                logger.info(f"📧 Password reset code sent to {payload.email} (username verified)")

                return MessageResponse(
                    ok=True,
                    message="E-posta ve kullanıcı adı doğrulandı. Şifre sıfırlama kodu gönderildi.",
                    expires_in_minutes=CODE_TTL_MINUTES,
                )

        # Başarısız (user yok veya username yanlış) — generic mesaj
        # (E-posta gönderilmez — bu sefer gerçekten kullanıcı doğrulatmadı)
        return MessageResponse(
            ok=False,
            message="E-posta veya kullanıcı adı eşleşmiyor. Bilgilerinizi kontrol edin.",
        )
    except HTTPException:
        raise
    except Exception as e:
        logger.exception("Forgot password step 1 error")
        raise HTTPException(500, f"Doğrulama hatası: {str(e)}")
...
async def forgot_password_reset(payload: ResetPasswordPayload):
    """
    Adım 2: Kod + yeni şifre ile hesap şifresini güncelle.
    """
    try:
        sb_admin = get_supabase_admin()

        # 1. User var mı?
        result = sb_admin.table("profiles").select("id, username").eq("email", payload.email).maybe_single().execute()
        if not result.data:
            raise HTTPException(404, "Kullanıcı bulunamadı")

        user_id = result.data["id"]
        username = result.data.get("username", payload.email.split("@")[0])
        ...
        # 3. Supabase üzerinden şifre güncelle (admin API)
        try:
            ...
            logger.exception(f"Supabase password update failed: {e}")
            raise HTTPException(500, f"Şifre güncellenemedi: {str(e)}")

        logger.info(f"✅ Password reset successful: {payload.email}")
        ...
        logger.exception("Forgot password step 2 error")
        raise HTTPException(500, f"Şifre sıfırlama hatası: {str(e)}")